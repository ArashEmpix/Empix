...
function banall_list()
	local hash =  'gbanned'
	local list = redis:smembers(hash)
	local text = "Hammered Users For Elemental BOT\n\n"
	for k,v in pairs(list) do
    local user_info = redis:hgetall('user:'..v)
		if user_info and user_info.print_name then
			local print_name = string.gsub(user_info.print_name, "_", " ")
...
function silent_users_list(chat_id)
	local hash =  'silent_user:'..chat_id
	local list = redis:smembers(hash)
	local text = "Silented Users For ["..chat_id.." ]:\n\n"
...
function ban_by_reply(extra, success, result)
	...
	if is_momod2(result.from.peer_id, result.to.peer_id) then -- Ignore mods,owner,admin
		return "you can't kick mods or sudo and admins"
	end
...
function banall_by_reply(extra, success, result)
    ...
    if is_admin2(result.from.peer_id) then -- Ignore admins
		...
		chat_del_user(get_receiver(msg), 'user#id'..result.from.peer_id, ok_cb, false)
		channel_kick(get_receiver(msg), 'user#id'..result.from.peer_id, ok_cb, false)
		send_large_msg(chat, "User "..name.."["..result.from.peer_id.."] globally Hammered")
	else
		return
  end
end
